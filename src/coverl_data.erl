...
-type arg_hit() :: [boolean() | any].
-type guard_hit() :: [[boolean()] | false].
-type branch_hit() :: {cd, number(), number(), arg_hit(), guard_hit()}.

-type hit() :: {{module(), file(), number(), line | branch_hit()}, integer()}.
...
-spec export(file:name(), [module()]) -> ok | {error, term()}.
...
cd_spec(Module, CDID, ClauseNum, [{File, Line, ArgSpec, []}]) ->
...
false_guard_cd([AndGuardList | OrGuardTail]) ->
    ...
    || AndFalseGuard <- false_guard_cd(AndGuardList, [])
    , TailGuard <- false_guard_cd(OrGuardTail)
    ];
false_guard_cd([]) ->
    [[]].

false_guard_cd([_], Acc) ->
    [lists:reverse([false | Acc])];
false_guard_cd([_ | ArgMatchTail], Acc) ->
    [lists:reverse([false | Acc]) | false_guard_cd(ArgMatchTail, [true | Acc])].
